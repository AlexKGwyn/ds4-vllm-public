...
## 4.1 Verify serving (gate)

```bash
# from box1:
distrobox enter vllm -- bash -lc 'ray status'      # 2 nodes / 2.0 GPU
curl -s http://127.0.0.1:1234/v1/models            # lists deepseek-v4-flash
curl -s http://127.0.0.1:1234/v1/chat/completions -H 'content-type: application/json' \
  -d '{"model":"deepseek-v4-flash","messages":[{"role":"user","content":"2+2? one word."}]}'
```
- Confirm RDMA is actually in use (not the TCP fallback): the serve journal
  must contain `tbv_ar2: rank<n> ready (qpn=…)` — the bringup's verify step
  greps for exactly this. (`tbv_ar init failed: slot tensor not page-aligned`
  is the inert v1 and expected.)

---

## 5. Quality gate (project rule — do not skip on any perf change)

This build's hard rule: **performance/serving changes must cost ZERO output
quality.** If you touch any kernel/tuning knob (`DS4_MOE_*`, indexer, MTP,
all-reduce), validate against golden outputs before trusting it: fixed-seed
temperature-0 prompts diffed pre/post, plus needle/recall probes at your
target context depth. **§0.2** lists what keeps recall correct.

## 6. Troubleshooting map

| symptom | cause → fix |
|---|---|
| box hard-hangs / oops on cable connect | stock `thunderbolt_net` over patched core → rebuild the **matched** net (§1.1); never mix |
| `usb4_rdma*` never appears; `insmod: Invalid module format` | kernel updated → module vermagic stale → rebuild (§1.1–1.2), restart `tbv-roce` on **both** |
| `ibv_devices` empty (esp. in container) | rail not named `usb4_rdma*`, or provider missing in container → `rdma dev set <d> name usb4_rdma0`; rebuild the image (it builds/ships the provider) |
| `gids/1` all zero | `thunderbolt0` has no `192.168.100.x` IP → bring the IP up first |
| `failed to allocate Rx HopID` / `tunnel enable … EBUSY` / `native HELLO no matching peer` | HopID allocator wedged by staggered/live reloads → **coordinated reboot of both boxes** (or unplug/replug cable) |
| decode slow; no `tbv_ar2: rank<n> ready` in the serve journal | RDMA all-reduce not up (TCP path) → fix RDMA (§1.5); the v1 `tbv_ar init failed: slot tensor not page-aligned` line is inert and expected |
| startup 25+ min, both ranks 100% CPU, GPU idle | cold Triton/inductor kernel caches (first bringup after a cache wipe) → wait it out; caches are pinned under `$HOME` so it is one-time |
| ranks behave differently / gibberish | `ds4-cluster-env.sh` not identical on both boxes → sync it |
| ds4 flip hangs at "placement group" | box2 container state wedged → `podman stop -t0 vllm && podman start vllm` on box2 |
| one or more cores pegged at 100% forever, even idle with the GPU at 0% | ROCr idle spinners (`AsyncEventsLoop`, indefinite active waits, and `amd::HostcallListener::Thread` polling the hostcall doorbell) → the image must carry the ROCr rebuilt with AMD's event-age series (`container/Dockerfile`), and `ds4-cluster-env.sh` must preload it; check `DS4_HSA_PRELOAD` is not 0 and that the container is not an older hand-built one |

## 7. Safety rules (operating a live box)

- **Never restart/bounce a service while it's serving requests** without
  checking for in-flight work and confirming with the owner.
- **Never `git checkout` a kernel rev on a serving box** (hard-hang), and never
  live-reload the thunderbolt **core**.
- RDMA recovery is a **both-boxes coordinated reboot** — plan for the downtime;
  don't try to fix it with more live reloads.
