"""Deterministic row-wise top-k with ascending output, for the DS4 sparse indexer.

Replaces the two ``torch.sort`` calls in
``vllm/v1/attention/ops/rocm_aiter_mla_sparse.py``:

  * ``_topk_indices_torch``        — full stable descending sort of every row
  * ``_canonicalize_topk_indices_`` — second sort, to put the selection ascending

with one Triton kernel that selects the top ``k`` entries of each row under the
total order ``(value descending, column index ascending)`` and writes them
**already sorted ascending by column index**, ``-1``-padded at the tail.
...
Conventions follow ``ds4_tl_indexer.py``: standalone venv top-level module, an
env kill-switch, and — the point of the bucketing discipline there — **no
shape-dependent compile key**. Every size is a runtime argument marked
...
# Kill-switch: DS4_TOPK=0 restores the torch two-sort path.
_ENABLED = os.environ.get("DS4_TOPK", "1") != "0"
...
# test_topk.py checks that.
_HIST_MODE = os.environ.get("DS4_TOPK_HIST", "hist")
...
def select_topk(
    ...
    """Entry point used by ``rocm_aiter_mla_sparse``. Honours ``DS4_TOPK=0``."""
