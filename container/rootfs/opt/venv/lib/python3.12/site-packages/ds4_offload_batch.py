...
which no small tier can ever satisfy. The observed symptom — a 182K prefill
against a 256 MiB tier refusing stores — reads like "the tier must be sized to
the in-flight sequence", but the tier is a proper LRU cache with refcounting:
``TieringOffloadingManager.complete_store`` pins a block via
...
latency, not the sequence — which is what makes a 15 GB disk cache affordable
behind a few hundred MiB of RAM.
...
FRAC_ENV = "DS4_OFFLOAD_STORE_BATCH_FRAC"
PROMOTE_FRAC_ENV = "DS4_OFFLOAD_PROMOTE_FRAC"
...
# recomputes everything anyway -- after paying the full disk read. Measured on
# a 4,645-token prompt against a 258-block tier: 258 blocks (512 MiB) read from
# disk, "cannot store blocks", and zero hits.
#
...
def resolve_store_batch_tokens(
    ...
    Args:
        ...
        frac: Fraction of the tier to spend per step. Defaults to
            ``$DS4_OFFLOAD_STORE_BATCH_FRAC`` then ``DEFAULT_FRAC``. A value
...
def resolve_promote_block_budget(
    ...
    a full-attention group on the same 4,645 tokens), so it dominates the rate
    and collapses the budget to a few hundred tokens -- about 6% of the prompt
    -- even though its lookup only ever touches a bounded tail. Counting the
    promotions themselves needs no model of any of that.

    Applies to the full-attention prefix scan only; see the call site.
    ``DS4_OFFLOAD_PROMOTE_FRAC=0`` restores stock behaviour (promote the whole
